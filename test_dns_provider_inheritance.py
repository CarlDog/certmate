...
def test_dns_provider_inheritance():
    ...
    test_settings = {
        ...
        "dns_provider": "route53",
        "default_accounts": {"cloudflare": "default", "route53": "certmate_test"},
    }
    ...
    for domain, expected_provider in test_cases:
        ...
        print(
            f"  {status} {domain} -> {detected_provider} (expected: {expected_provider})"
        )
    ...
    for domain, expected_provider in suggestion_test_cases:
        suggested_provider, confidence = suggest_dns_provider_for_domain(
            domain, test_settings
        )
        status = "✓" if suggested_provider == expected_provider else "✗"
        print(
            f"  {status} {domain} -> {suggested_provider} (confidence: {confidence}%, expected: {expected_provider})"
        )
    ...
    def simulate_dns_provider_selection(domain, provided_dns_provider, settings):
        ...
        if existing_provider and existing_provider != settings.get(
            "dns_provider", "cloudflare"
        ):
    ...
    for domain, provided_provider, expected_provider in creation_test_cases:
        selected_provider = simulate_dns_provider_selection(
            domain, provided_provider, test_settings
        )
        status = "✓" if selected_provider == expected_provider else "✗"
        print(
            f"  {status} {domain} (provided: {provided_provider}) -> {selected_provider} (expected: {expected_provider})"
        )
