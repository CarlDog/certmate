...
def test_current_certificate_dns_providers():
    ...
    try:
        ...
        response = requests.get(
            f"{BASE_URL}/api/certificates", headers=HEADERS, timeout=10
        )
            ...
            for domain, expected_provider in expected_providers.items():
                ...
                for cert in certificates:
                    if cert.get("domain") == domain:
                        ...
                        print(
                            f"{status} {domain}: {actual_provider} (expected: {expected_provider})"
                        )
                        found = True
                        break
        ...
        if response.status_code == 200:
            ...
            for i, domain in enumerate(domains):
                if isinstance(domain, dict):
                    ...
                    print(
                        f"  {i+1}. {domain} (old format - will inherit global DNS provider)"
                    )
                else:
