...
def test_csr_operations():
    ...
    assert (
        info and info.get("common_name") == "test.example.com"
    ), "Invalid CN in CSR info"
...
def test_certificate_lifecycle():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert (
            metadata and metadata["common_name"] == "user@example.com"
        ), "Invalid metadata"

        # Test 4: Certificate file download
        cert_file = manager.get_certificate_file(cert_id, "crt")
        assert (
            cert_file and b"BEGIN CERTIFICATE" in cert_file
...
def test_filtering_and_search():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        ), f"Expected 1 search result, got {len(search_results)}"

        # Test 3: Multi-filter
        vpn_certs = manager.list_client_certificates(cert_usage="vpn", revoked=False)
...
def test_batch_operations():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        certs = manager.list_client_certificates()
        assert (
            len(certs) == batch_count
        ), f"Expected {batch_count} certs, got {len(certs)}"
...
def test_ocsp_and_crl():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert (
            status and status["status"] == "good"
        ), f"Expected good status, got {status}"

        # Test 2: OCSP response generation
        response = ocsp.generate_ocsp_response(status)
        assert (
            response and response.get("response_status") == "successful"
        ...
        assert crl_der is None or isinstance(
            crl_der, bytes
        ), "CRL DER conversion returned unexpected type"

        # Test 5: Revoke and check OCSP
        manager.revoke_certificate(cert_id, reason="Testing")
        revoked_status = ocsp.get_cert_status(serial)
        assert (
            revoked_status and revoked_status["status"] == "revoked"
        ), "OCSP status not updated after revocation"
