...
class EndpointTester:
    ...
    def test_endpoint(
        ...
        try:
            ...
            if response.status_code in expected_codes:
                ...
                return False, "✗ 404 Not Found", response.status_code
            elif response.status_code == 405:
                return False, "✗ 405 Method Not Allowed", response.status_code
            else:
    ...
    def run_all_tests(self):
        ...
        print(
            f"{Colors.BOLD}{Colors.CYAN}🚀 CertMate API Endpoint Test Suite{Colors.RESET}"
        )
        print(f"{Colors.WHITE}Testing API at: {self.base_url}{Colors.RESET}")
        print(
            f"{Colors.WHITE}Timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}{Colors.RESET}"
        )
        print("=" * 80)
        ...
        if not self.api_token:
            ...
            print(
                f"{Colors.YELLOW}   Use --token <your-token> to test authenticated endpoints.{Colors.RESET}"
            )
        else:
    ...
    def _test_health_endpoints(self):
        ...
        for method, path, desc, data, expected, auth in tests:
            success, message, status = self.test_endpoint(
                method, path, desc, data, expected, auth
            )
    ...
    def _test_settings_endpoints(self):
        ...
        for method, path, desc, data, expected in tests:
            success, message, status = self.test_endpoint(
                method, path, desc, data, expected
            )
            self._record_result(method, path, desc, success, message, status)
    ...
    def _test_certificate_endpoints(self):
        ...
        tests = [
            ...
            (
                "GET",
                "/api/certificates/test.example.com/download",
            ...
            (
                "POST",
                "/api/certificates/test.example.com/renew",
                "Renew Certificate",
                None,
                [200, 404, 422, 500],
            ),
    ...
    def _test_backup_endpoints(self):
        ...
        tests = [
            ...
            (
                ...
                {"type": "certificates", "reason": "api_test"},
                [200, 201, 400],
            ),
            (
                "POST",
                ...
                try:
                    ...
                    if response.status_code in [200, 201]:
                        ...
                        if "created_backups" in result and result["created_backups"]:
                            backup_filename = result["created_backups"][0].get(
                                "filename"
                            )
                except Exception:
                    pass
        ...
        if backup_filename:
            restore_tests = [
                (
                    "POST",
                    "/api/backups/restore/settings",
                    "Restore Settings Backup",
                    {"filename": backup_filename, "create_backup_before_restore": True},
                    [200, 400, 404],
                ),
            ]
            ...
            success, message, status = self.test_endpoint(
                "POST",
                "/api/backups/restore/settings",
                "Restore Settings Backup (No File)",
    ...
    def _test_metrics_endpoints(self):
        ...
        tests = [
            ...
            (
                ...
                None,
                [200, 503],
            ),  # 503 if prometheus_client not available
        ]
        ...
        if self.api_token:
            web_tests_auth = [
                (
                    ...
                    [200, 401],
                    False,
                ),  # No auth for initial setup
                (
                    "GET",
                    "/api/web/certificates",
                    "Web Certificates API",
                    None,
                    [200, 401],
                    True,
                ),
    ...
    def _record_result(
        ...
        print(
            f"  {status_icon} {color}{method:4} {path:40} {description:30} {message}{Colors.RESET}"
        )
        ...
        if failed_tests > 0:
            ...
            for result in self.results:
                if not result["success"]:
                    print(
                        f"  • {result['method']} {result['path']} - {result['message']}"
                    )
...
def main():
    ...
    if not args.public_only:
        if args.token:
            ...
            if api_token:
                print(
                    f"{Colors.GREEN}✓ Loaded API token from settings.json{Colors.RESET}"
                )
            else:
                print(
                    f"{Colors.YELLOW}⚠️  Could not load API token from settings.json{Colors.RESET}"
                )

    # Create tester instance
    tester = EndpointTester(
        base_url=args.url, api_token=api_token, quick_mode=args.quick
    )
