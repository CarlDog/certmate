...
$Table = "certsOS" #Also used for $ScriptName
$ScriptName = $Table # Name for logging purposes
$LogTable = "collectionLogs"
$ErrorActionPreference = 'Continue'

# =====================
# OS Certificate Collection Parameters
# =====================
$StoreLocations = @("LocalMachine")
$StoreNames = @("My", "Root", "CA", "AuthRoot") # Common OS stores
...
$CERTIFICATE_OBJECT_TEMPLATE = {
    ...
    [pscustomobject]@{
        ...
        Notes         = $null # Initialize Notes property
    }
}
#endregion
...
function Get-RemoteOSCertificates {
    param (
        [string]$machineName,
    ...
    try {
        ...
        foreach ($cert in $certificates) {
            if ([string]::IsNullOrEmpty($cert.Thumbprint)) {
                $machine = if ($cert.MachineName) { $cert.MachineName } else { "<unknown>" }
                $storeLoc = if ($cert.StoreLocation) { $cert.StoreLocation } else { "<unknown>" }
                $storeName = if ($cert.StoreName) { $cert.StoreName } else { "<unknown>" }
                $subject = if ($cert.Subject) { $cert.Subject } else { "<unknown>" }
                Write-Log -Message ("Skipping certificate with null or empty thumbprint. Machine: {0}, Store: {1}\\{2}, Subject: {3}" -f $machine, $storeLoc, $storeName, $subject) -LogLevel "WARNING"
                continue
            }
...
#region ScriptMain
try {
    Write-Log -Message ("Starting OS certificate collection script execution") -LogLevel "INFO"
    ...
    if ($filteredMachines -and $filteredMachines.Count -gt 0) {
        Write-Log -Message ("Found {0} machines to scan for OS certificates" -f $filteredMachines.Count) -LogLevel "INFO"
        $allCertificates = @()
        foreach ($machine in $filteredMachines) {
            ...
            if (-not (Test-WinRMPort -ComputerName $machineName -Port 5985)) {
                Write-Log -Message ("Skipping $machineName because WinRM port is not accessible") -LogLevel "WARNING"
                continue
            }
            Write-Log -Message ("Scanning OS certificate stores on {0}" -f $machineName) -LogLevel "INFO"
            $certs = Get-RemoteOSCertificates -machineName $machineName -storeLocations $StoreLocations -storeNames $StoreNames -filterScript $filterScript -archivedThumbprints $archivedThumbprints
            if ($certs) {
                $allCertificates += $certs
            }
        }
        if ($allCertificates.Count -gt 0) {
            Write-Log -Message ("Updating SQL database with {0} OS certificates" -f $allCertificates.Count) -LogLevel "INFO"
            Update-CertificateDatabase -certificates $allCertificates
        } else {
            Write-Log -Message ("No OS certificates found to insert into the database") -LogLevel "WARNING"
        }
    } else {
        Write-Log -Message ("No machines found in machineList table matching filter.") -LogLevel "WARNING"
    }
    Write-Log -Message ("OS certificate collection script completed successfully") -LogLevel "INFO"
} catch {
    $errorMessage = "ERROR: {0}`nStackTrace: {1}" -f $_.Exception.Message, $_.ScriptStackTrace
    Write-Log -Message $errorMessage -LogLevel "ERROR"
    Write-Log -Message ("OS certificate collection script failed with errors") -LogLevel "ERROR"
} finally {
