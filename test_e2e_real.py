...
class TestRunner:
    ...
    def run_test(
        self, name: str, test_func: Callable, severity: Severity = Severity.MEDIUM
    ) -> TestResult:
...
def test_server_health(runner: TestRunner) -> Tuple[bool, str, Dict]:
    ...
    return (
        True,
        "Server healthy",
        {"version": data.get("version"), "duration": duration},
    )
...
def test_list_certificates(runner: TestRunner) -> Tuple[bool, str, Dict]:
    ...
    try:
        ...
        return (
            True,
            f"Listed {cert_count} certificates",
            {"count": cert_count, "duration": duration},
        )
    except Exception as e:
...
def test_dns_provider_configured(runner: TestRunner) -> Tuple[bool, str, Dict]:
    ...
    if runner.config.dns_account_id in cloudflare_accounts:
        ...
        if has_token:
            return (
                True,
                f"Cloudflare account '{runner.config.dns_account_id}' configured",
                {},
            )
    ...
    return (
        ...
        {"dns_accounts": list(cloudflare_accounts.keys())},
    )
...
def test_create_certificate_staging(runner: TestRunner) -> Tuple[bool, str, Dict]:
    ...
    if response.status_code == 201:
        ...
        return (
            True,
            f"Certificate created for {test_domain}",
            {"domain": test_domain, "duration": duration},
        )
    ...
    try:
        ...
        return (
            False,
            f"Creation failed: {error.get('error', response.status_code)}",
            {
                "domain": test_domain,
...
def test_download_certificate(runner: TestRunner) -> Tuple[bool, str, Dict]:
    ...
    if response.status_code == 200:
        ...
        return (
            ...
            {"domain": domain, "size": content_length, "duration": duration},
        )
...
def test_certificate_status(runner: TestRunner) -> Tuple[bool, str, Dict]:
    ...
    if response.status_code == 200:
        ...
        return (
            True,
            f"Certificate status: {data.get('status', 'unknown')}",
            {
                "domain": domain,
...
def test_client_ca_exists(runner: TestRunner) -> Tuple[bool, str, Dict]:
    ...
    if response.status_code == 200:
        ...
        return (
            True,
            "Client CA exists",
            {"serial": data.get("serial"), "subject": data.get("subject")},
        )
...
def test_generate_client_cert(runner: TestRunner) -> Tuple[bool, str, Dict]:
    ...
    response, duration = runner.api_call(
        "POST", "/api/client-certificates", json=payload
    )
    ...
    if response.status_code in (200, 201):
        ...
        return (
            True,
            f"Client cert created: {payload['common_name']}",
            {
...
def test_list_client_certs(runner: TestRunner) -> Tuple[bool, str, Dict]:
    ...
    if response.status_code == 200:
        ...
        return (
            True,
            f"Listed {count} client certificates",
...
def test_health_response_time(runner: TestRunner) -> Tuple[bool, str, Dict]:
    ...
    return (
        True,
        f"Response time OK: avg={avg_time:.3f}s",
        {"times": times, "avg": avg_time},
    )
...
def test_backup_creation(runner: TestRunner) -> Tuple[bool, str, Dict]:
    ...
    if response.status_code in (200, 201):
        ...
        return (
            ...
            {"filename": data.get("filename"), "duration": duration},
        )
...
def test_web_ui_accessible(runner: TestRunner) -> Tuple[bool, str, Dict]:
    ...
    if response.status_code == 200:
        ...
        return (
            True,
            f"Root endpoint accessible (type: {content_type})",
            {"duration": duration},
        )
...
def cleanup_cloudflare_dns_records(runner: TestRunner):
    ...
    try:
        ...
        cleanup_records = [
            r for r in records if r.get("name", "").startswith("_acme-challenge.e2e-")
        ]
        ...
        print(
            f"  [INFO] Cleanup complete: {deleted}/{len(cleanup_records)} records deleted"
        )
...
def run_all_tests(
    ...
    print("-" * 40)
    runner.run_test(
        "Server Health", lambda: test_server_health(runner), Severity.CRITICAL
    )
    runner.run_test(
        "Web UI Accessible", lambda: test_web_ui_accessible(runner), Severity.LOW
    )
    ...
    print("-" * 40)
    runner.run_test(
        "API Authentication", lambda: test_api_authentication(runner), Severity.CRITICAL
    )
    runner.run_test(
        "Invalid Token Rejected", lambda: test_invalid_token(runner), Severity.HIGH
    )
    ...
    runner.run_test(
        ...
        lambda: test_dns_provider_configured(runner),
        Severity.CRITICAL,
    )
    ...
    print("-" * 40)
    runner.run_test(
        "List Certificates", lambda: test_list_certificates(runner), Severity.HIGH
    )
    runner.run_test(
        "Settings Endpoint", lambda: test_settings_endpoint(runner), Severity.MEDIUM
    )

    if not skip_creation and not quick_mode:
        runner.run_test(
            "Create Certificate (Staging)",
            lambda: test_create_certificate_staging(runner),
            Severity.CRITICAL,
        )
        ...
        if runner.created_certificates:
            runner.run_test(
                ...
                lambda: test_download_certificate(runner),
                Severity.HIGH,
            )
            runner.run_test(
                "Certificate Status",
                lambda: test_certificate_status(runner),
                Severity.MEDIUM,
            )
    else:
        runner.skip_test(
            "Create Certificate",
            "Skipped (--skip-creation or --quick)",
            Severity.CRITICAL,
        )
        runner.skip_test(
            "Download Certificate", "Skipped (no cert created)", Severity.HIGH
        )
        runner.skip_test(
            "Certificate Status", "Skipped (no cert created)", Severity.MEDIUM
        )
    ...
    print("-" * 40)
    runner.run_test(
        "Client CA Exists", lambda: test_client_ca_exists(runner), Severity.MEDIUM
    )
    runner.run_test(
        "List Client Certs", lambda: test_list_client_certs(runner), Severity.MEDIUM
    )

    if not quick_mode:
        runner.run_test(
            "Generate Client Cert",
            lambda: test_generate_client_cert(runner),
            Severity.MEDIUM,
        )
    ...
    runner.run_test(
        ...
        lambda: test_invalid_domain_format(runner),
        Severity.MEDIUM,
    )
    runner.run_test(
        "Missing Required Fields",
        lambda: test_missing_required_fields(runner),
        Severity.MEDIUM,
    )
    runner.run_test(
        "Non-existent Certificate",
        lambda: test_nonexistent_certificate(runner),
        Severity.MEDIUM,
    )
    ...
    runner.run_test(
        ...
        lambda: test_health_response_time(runner),
        Severity.MEDIUM,
    )
    ...
    print("-" * 40)
    runner.run_test(
        "List Backups", lambda: test_backup_listing(runner), Severity.MEDIUM
    )

    if not quick_mode:
        runner.run_test(
            "Create Backup", lambda: test_backup_creation(runner), Severity.MEDIUM
        )
        runner.run_test(
            "Audit Log", lambda: test_audit_log_endpoint(runner), Severity.LOW
        )
    else:
...
def main():
    ...
    parser.add_argument(
        "--url", default="http://localhost:8000", help="CertMate server URL"
    )
    parser.add_argument(
        "--token", default="", help="API bearer token (or uses settings.json)"
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Use Let's Encrypt staging (default)",
    )
    parser.add_argument(
        "--production",
        action="store_true",
        help="Use production Let's Encrypt (careful!)",
    )
    parser.add_argument("--quick", action="store_true", help="Quick smoke test only")
    parser.add_argument(
        "--skip-creation", action="store_true", help="Skip certificate creation"
    )
    parser.add_argument(
        "--no-cleanup", action="store_true", help="Don't cleanup DNS records"
    )
