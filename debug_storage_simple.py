...
def test_storage_backend_manually():
    ...
    # Test data
    test_data = {"backend": "local_filesystem", "config": {"cert_dir": "certificates"}}
    ...
    response = client.post(
        "/api/storage/test", headers=headers, data=json.dumps(test_data)
    )
    ...
    response2 = client.post(
        "/api/storage/test", headers=headers, data=json.dumps(test_data_azure)
    )
