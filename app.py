...
log_level = getattr(
    logging, os.getenv("CERTMATE_LOG_LEVEL", "INFO").upper(), logging.INFO
)
...
class CertMateApp:
    ...
    def _setup_directories(self):
        ...
        try:
            ...
            logger.warning("Using temporary directories - data may not persist")
    ...
    def _setup_api(self):
        ...
        try:
            ...
            self.api_resources = create_api_resources(
                self.api, self.api_models, self.managers
            )

            # Create client certificate API resources
            self.api_resources.update(
                create_client_certificate_resources(self.api, self.managers)
            )

            # Create namespaces
            ns_certificates = Namespace(
                "certificates", description="Certificate operations"
            )
            ns_client_certs = Namespace(
                "client-certs", description="Client certificate operations"
            )
            ...
            ns_backups = Namespace(
                "backups", description="Backup and restore operations"
            )
            ns_cache = Namespace("cache", description="Cache management operations")
            ns_metrics = Namespace(
                "metrics", description="Prometheus metrics and monitoring"
            )
            ...
            ns_settings.add_resource(
                self.api_resources["DNSProviders"], "/dns-providers"
            )
            ns_settings.add_resource(
                self.api_resources["CAProviderTest"], "/test-ca-provider"
            )
            ...
            ns_certificates.add_resource(
                self.api_resources["CreateCertificate"], "/create"
            )
            ns_certificates.add_resource(
                self.api_resources["DownloadCertificate"], "/<string:domain>/download"
            )
            ns_certificates.add_resource(
                self.api_resources["RenewCertificate"], "/<string:domain>/renew"
            )
            ns_backups.add_resource(self.api_resources["BackupList"], "")
            ns_backups.add_resource(self.api_resources["BackupCreate"], "/create")
            ns_backups.add_resource(
                self.api_resources["BackupDownload"],
                "/download/<backup_type>/<filename>",
            )
            ns_backups.add_resource(
                self.api_resources["BackupRestore"], "/restore/<backup_type>"
            )
            ns_backups.add_resource(
                self.api_resources["BackupDelete"], "/delete/<backup_type>/<filename>"
            )

            # Client Certificate endpoints
            ns_client_certs.add_resource(
                self.api_resources["ClientCertificateList"], ""
            )
            ns_client_certs.add_resource(
                self.api_resources["ClientCertificateCreate"], "/create"
            )
            ns_client_certs.add_resource(
                self.api_resources["ClientCertificateDetail"], "/<string:identifier>"
            )
            ns_client_certs.add_resource(
                self.api_resources["ClientCertificateDownload"],
                "/<string:identifier>/download/<string:file_type>",
            )
            ns_client_certs.add_resource(
                self.api_resources["ClientCertificateRevoke"],
                "/<string:identifier>/revoke",
            )
            ns_client_certs.add_resource(
                self.api_resources["ClientCertificateRenew"],
                "/<string:identifier>/renew",
            )
            ns_client_certs.add_resource(
                self.api_resources["ClientCertificateStatistics"], "/stats"
            )
            ns_client_certs.add_resource(
                self.api_resources["ClientCertificateBatch"], "/batch"
            )

            # OCSP endpoints
            ns_ocsp.add_resource(
                self.api_resources["OCSPStatus"], "/status/<int:serial_number>"
            )

            # CRL endpoints
            ns_crl.add_resource(
                self.api_resources["CRLDistribution"], "/download/<string:format_type>"
            )
    ...
    def run(self, host="0.0.0.0", port=5000, debug=False):
        ...
        try:
            ...
            for directory in dirs_to_check:
                ...
                logger.info(
                    f"  {status} {directory.name}: {'exists' if exists else 'missing'}{', writable' if writable else ', not writable' if exists else ''}"
                )
            ...
            try:
                ...
                logger.info(
                    f"  🔑 API Token: {'✅ set' if api_token_set else '❌ using default'}"
                )

                if api_token_set:
                    token = settings.get("api_bearer_token", "")
                    masked_token = (
                        f"{token[:8]}{'*' * (len(token) - 12)}{token[-4:]}"
                        if len(token) > 12
                        else "****"
                    )
            ...
            logger.info(
                f"  ❤️  Health:    http://{host if host != '0.0.0.0' else 'localhost'}:{port}/health"
            )
            ...
            logger.info(
                "  • Configure your DNS provider in Settings before creating certificates"
            )
            ...
            self.app.run(
                host=host, port=port, debug=debug, threaded=True, use_reloader=debug
            )
    ...
    def decorated_function(*args, **kwargs):
        try:
            ...
            if not expected_token:
                return {
                    "error": "Server configuration error: no API token configured",
...
def create_certificate(
    domain, email, dns_provider=None, dns_config=None, account_id=None, staging=False
):
...
def create_certificate_legacy(domain, email, cloudflare_token):
    ...
    try:
        result = certmate_app.managers["certificates"].create_certificate_legacy(
            domain, email, cloudflare_token
        )
...
def migrate_dns_providers_to_multi_account(settings):
    ...
    return certmate_app.managers["settings"].migrate_dns_providers_to_multi_account(
        settings
    )
...
def get_dns_provider_account_config(provider, account_id=None, settings=None):
    ...
    if settings is None:
        ...
        settings = load_settings()
    return certmate_app.managers["dns"].get_dns_provider_account_config(
...
def suggest_dns_provider_for_domain(domain, settings=None):
    ...
    return certmate_app.managers["dns"].suggest_dns_provider_for_domain(
        domain, settings
    )
