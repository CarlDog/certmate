...
def test_storage_response():
    ...
    test_data = {
        ...
        "config": {"cert_dir": "test_certificates"},
    }

    # Make the request
    response = client.post(
        "/api/storage/test", headers=auth_headers, data=json.dumps(test_data)
    )
    ...
    response2 = client.post(
        "/api/storage/test", headers=auth_headers, data=json.dumps(test_data_azure)
    )
