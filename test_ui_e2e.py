...
class TestSuite:
    ...
    def print_summary(self):
        ...
        if self.failed == 0:
            ...
            print(
                colorize(f"  ⚠️  {self.failed} TEST(S) FAILED", Colors.RED, Colors.BOLD)
            )
        ...
        print(
            f"    Total Tests:    {colorize(str(self.total), Colors.WHITE, Colors.BOLD)}"
        )
        print(f"    Passed:         {colorize(str(self.passed), Colors.GREEN)} ✓")
        print(
            f"    Failed:         {colorize(str(self.failed), Colors.RED if self.failed > 0 else Colors.GREEN)} {'✗' if self.failed > 0 else '✓'}"
        )
        print(
            f"    Pass Rate:      {colorize(f'{self.pass_rate:.1f}%', pass_color, Colors.BOLD)}"
        )
            ...
            for r in self.results:
                if not r.passed:
                    ...
                    if r.screenshot_path:
                        print(
                            colorize(
                                f"      📸 Screenshot: {r.screenshot_path}",
                                Colors.YELLOW,
                            )
                        )
...
class CertMateUITests:
    ...
    def test_context(self, name: str, category: str):
        ...
        try:
            ...
            if self.page:
                screenshot_path = str(
                    self.screenshot_dir
                    / f"fail_{name.replace(' ', '_')}_{int(time.time())}.png"
                )
                try:
    ...
    def run_all(self) -> bool:
        ...
        with sync_playwright() as p:
            ...
            print_success("Browser launched (Chromium)")
    ...
    def _test_homepage_elements(self):
        ...
        with self.test_context("Create Certificate button exists", "Homepage"):
            button = self.page.locator(
                'button[type="submit"]:has-text("Create Certificate")'
            )
    ...
    def _test_api_integration(self):
        ...
        with self.test_context("Settings API endpoint", "API"):
            ...
            assert response.status in [
                ...
                401,
                403,
            ], f"Settings API error: {response.status}"
        ...
        with self.test_context("Certificates list API", "API"):
            ...
            assert response.status in [
                ...
                401,
                403,
            ], f"Certificates API error: {response.status}"
...
Examples:
  ...
  python test_ui_e2e.py --slow-mo 500      # Slow down for debugging
    ...
    parser.add_argument(
        "--headed", "-H", action="store_true", help="Run with visible browser window"
    )
