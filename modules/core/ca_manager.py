...
class CAManager:
    ...
    def get_ca_config(
        self, ca_provider: str, account_id: str = None
        ...
        if "accounts" in provider_config:
            ...
            if account_id:
                if account_id not in accounts:
                    raise ValueError(
                        f"Account '{account_id}' not found for CA provider '{ca_provider}'"
                    )
                ...
                if default_account_id in accounts:
                    ...
                    if accounts:
                        ...
                        raise ValueError(
                            f"No accounts configured for CA provider '{ca_provider}'"
                        )
        else:
    ...
    def get_eab_credentials(
        self, ca_provider: str, account_config: Dict[str, Any]
    ) -> Tuple[str, str]:
        ...
        if not eab_key_id or not eab_hmac_key:
            raise ValueError(
                f"EAB credentials not configured for CA provider '{ca_provider}'"
            )

        return eab_key_id, eab_hmac_key

    def create_ca_trust_bundle(
        self, ca_provider: str, account_config: Dict[str, Any] = None
    ) -> Optional[str]:
    ...
    def build_certbot_command(
        ...
        if self.requires_eab(ca_provider):
            eab_key_id, eab_hmac_key = self.get_eab_credentials(
                ca_provider, account_config
            )
    ...
    def validate_ca_configuration(
        self, ca_provider: str, config: Dict[str, Any]
    ) -> Tuple[bool, str]:
        """Validate CA provider configuration"""
        if ca_provider not in self.ca_providers:
            return False, f"Unsupported CA provider: {ca_provider}"

        self.ca_providers[ca_provider]
    ...
    def get_ca_account_display_info(
        self, ca_provider: str, config: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Get display-friendly information about CA account"""
        display_info = {
            "provider_name": self.ca_providers.get(ca_provider, {}).get(
                "name", ca_provider
            ),
            "account_name": config.get("name", "Default Account"),
            "description": config.get("description", ""),
            "certificate_types": self.ca_providers.get(ca_provider, {}).get(
                "certificate_types", []
            ),
            "supports_wildcard": self.ca_providers.get(ca_provider, {}).get(
