...
class DNSManager:
    ...
    def get_dns_provider_account_config(self, provider, account_id=None, settings=None):
        ...
        try:
            ...
            if "accounts" in provider_config:
                ...
                if account_id:
                    if account_id in accounts:
                        ...
                        logger.warning(
                            f"Account '{account_id}' not found for provider '{provider}'"
                        )
                        return None, None
                ...
                for acc_id, acc_config in accounts.items():
                    if isinstance(acc_config, dict) and any(
                        key in acc_config
                        for key in [
                            "api_token",
                            "access_key_id",
            ...
            logger.error(
                f"Error getting DNS provider account config for {provider}: {e}"
            )
            return None, None
    ...
    def suggest_dns_provider_for_domain(self, domain, settings=None):
        ...
        if any(
            ...
            for pattern in ["aws", "amazon", "route53", "test.certmate.org"]
        ):
            return "route53", 70
        ...
        if any(
            pattern in domain_lower
            for pattern in ["cf-", "cloudflare", "audiolibri.org"]
        ):
    ...
    def create_dns_account(self, provider, account_id, account_config, settings=None):
        ...
        try:
            ...
            success = self.settings_manager.save_settings(
                settings, f"dns_account_create_{provider}_{account_id}"
            )

            if success:
                logger.info(
                    f"Created/updated DNS account '{account_id}' for provider '{provider}'"
                )
    ...
    def delete_dns_account(self, provider, account_id, settings=None):
        ...
        try:
            ...
            # Save settings
            success = self.settings_manager.save_settings(
                settings, f"dns_account_delete_{provider}_{account_id}"
            )

            if success:
                logger.info(
                    f"Deleted DNS account '{account_id}' for provider '{provider}'"
                )
    ...
    def set_default_account(self, provider, account_id, settings=None):
        ...
        try:
            ...
            # Save settings
            success = self.settings_manager.save_settings(
                settings, f"dns_default_account_{provider}_{account_id}"
            )

            if success:
                logger.info(
                    f"Set default DNS account '{account_id}' for provider '{provider}'"
                )
