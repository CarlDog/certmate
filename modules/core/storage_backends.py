...
class CertificateStorageBackend(ABC):
    ...
    def store_certificate(
        self, domain: str, cert_files: Dict[str, bytes], metadata: Dict[str, Any]
    ) -> bool:
...
class LocalFileSystemBackend(CertificateStorageBackend):
    ...
    def __init__(self, cert_dir: Path):
        ...
        logger.info(
            f"LocalFileSystemBackend initialized with cert_dir: {self.cert_dir}"
        )
...
class AzureKeyVaultBackend(CertificateStorageBackend):
    ...
    def __init__(self, config: Dict[str, str]):
        ...
        if not all(
            [self.vault_url, self.client_id, self.client_secret, self.tenant_id]
        ):
            raise ValueError(
                "Azure Key Vault backend requires vault_url, client_id, client_secret, and tenant_id"
            )
    ...
    def _get_client(self):
        ...
        if self._client is None:
            try:
                ...
                raise ImportError(
                    "Azure Key Vault backend requires 'azure-keyvault-secrets' and 'azure-identity' packages"
                )
        return self._client
    ...
    def store_certificate(
        ...
        try:
            ...
            for filename, content in cert_files.items():
                secret_name = self._sanitize_secret_name(
                    f"cert-{domain}-{filename.replace('.', '-')}"
                )
            ...
            logger.info(
                f"Certificate stored successfully in Azure Key Vault for {domain}"
            )
            return True

        except Exception as e:
            logger.error(
                f"Failed to store certificate in Azure Key Vault for {domain}: {e}"
            )
            return False
    ...
    def retrieve_certificate(
        ...
        try:
            ...
            logger.error(
                f"Failed to retrieve certificate from Azure Key Vault for {domain}: {e}"
            )
            return None
    ...
    def list_certificates(self) -> List[str]:
        ...
        try:
            ...
            for secret_properties in client.list_properties_of_secrets():
                if secret_properties.name.startswith(
                    "cert-"
                ) and secret_properties.name.endswith("-cert-pem"):
                    # Extract domain from secret name
                    domain = (
                        secret_properties.name.replace("cert-", "")
                        .replace("-cert-pem", "")
                        .replace("-", ".")
                    )
                    domains.add(domain)
    ...
    def delete_certificate(self, domain: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to delete certificate from Azure Key Vault for {domain}: {e}"
            )
            return False
...
class AWSSecretsManagerBackend(CertificateStorageBackend):
    ...
    def __init__(self, config: Dict[str, str]):
        ...
        if not all([self.access_key_id, self.secret_access_key]):
            raise ValueError(
                "AWS Secrets Manager backend requires access_key_id and secret_access_key"
            )
    ...
    def _get_client(self):
        ...
        if self._client is None:
            try:
                ...
                raise ImportError(
                    "AWS Secrets Manager backend requires 'boto3' package"
                )
        return self._client
    ...
    def store_certificate(
        ...
        try:
            ...
            logger.info(
                f"Certificate stored successfully in AWS Secrets Manager for {domain}"
            )
            return True

        except Exception as e:
            logger.error(
                f"Failed to store certificate in AWS Secrets Manager for {domain}: {e}"
            )
            return False
    ...
    def retrieve_certificate(
        ...
        try:
            ...
            cert_files = {
                k: v.encode("utf-8") for k, v in secret_data.get("files", {}).items()
            }
            ...
            logger.error(
                f"Failed to retrieve certificate from AWS Secrets Manager for {domain}: {e}"
            )
            return None
    ...
    def delete_certificate(self, domain: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to delete certificate from AWS Secrets Manager for {domain}: {e}"
            )
            return False
...
class HashiCorpVaultBackend(CertificateStorageBackend):
    ...
    def __init__(self, config: Dict[str, str]):
        ...
        if not all([self.vault_url, self.vault_token]):
            raise ValueError(
                "HashiCorp Vault backend requires vault_url and vault_token"
            )
    ...
    def store_certificate(
        ...
        try:
            ...
            logger.info(
                f"Certificate stored successfully in HashiCorp Vault for {domain}"
            )
            return True

        except Exception as e:
            logger.error(
                f"Failed to store certificate in HashiCorp Vault for {domain}: {e}"
            )
            return False
    ...
    def retrieve_certificate(
        ...
        try:
            ...
            logger.error(
                f"Failed to retrieve certificate from HashiCorp Vault for {domain}: {e}"
            )
            return None
        ...
        try:
            ...
            if self.engine_version == "v2":
                response = client.secrets.kv.v2.list_secrets(
                    path="certmate/certificates", mount_point=self.mount_point
                )
            else:
    ...
    def delete_certificate(self, domain: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to delete certificate from HashiCorp Vault for {domain}: {e}"
            )
            return False
...
class InfisicalBackend(CertificateStorageBackend):
    ...
    def __init__(self, config: Dict[str, str]):
        ...
        if not all([self.client_id, self.client_secret, self.project_id]):
            raise ValueError(
                "Infisical backend requires client_id, client_secret, and project_id"
            )
    ...
    def _get_client(self):
        ...
        if self._client is None:
            try:
                ...
                raise ImportError(
                    "Infisical backend requires 'infisical-python' package"
                )
        return self._client
    ...
    def retrieve_certificate(
        ...
        try:
            ...
            logger.error(
                f"Failed to retrieve certificate from Infisical for {domain}: {e}"
            )
            return None
    ...
    def list_certificates(self) -> List[str]:
        ...
        try:
            ...
            for secret in secrets:
                if secret.secret_name.startswith(
                    "certmate-"
                ) and secret.secret_name.endswith("-cert-pem"):
                    # Extract domain from secret name
                    domain = (
                        secret.secret_name.replace("certmate-", "")
                        .replace("-cert-pem", "")
                        .replace("-", ".")
                    )
                    domains.add(domain)
    ...
    def delete_certificate(self, domain: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to delete certificate from Infisical for {domain}: {e}"
            )
            return False
...
class StorageManager:
    ...
    def _initialize_backend(self):
        ...
        try:
            ...
            if backend_type == "local_filesystem":
                ...
                logger.warning(
                    f"Unknown storage backend: {backend_type}, falling back to local filesystem"
                )
            ...
            logger.info(
                f"Storage backend initialized: {self._backend.get_backend_name()}"
            )
    ...
    def migrate_certificates(
        self,
        source_backend: CertificateStorageBackend,
        target_backend: CertificateStorageBackend,
    ) -> Dict[str, bool]:
        ...
        try:
            ...
            logger.info(
                f"Starting migration of {len(domains)} certificates from {source_backend.get_backend_name()} to {target_backend.get_backend_name()}"
            )
            ...
            for domain in domains:
                try:
                    ...
                    if cert_data:
                        ...
                        # Store in target
                        success = target_backend.store_certificate(
                            domain, cert_files, metadata
                        )
                        migration_results[domain] = success
                        if success:
                            logger.info(
                                f"Successfully migrated certificate for {domain}"
                            )
                        else:
                        ...
                        logger.error(
                            f"Failed to retrieve certificate for {domain} from source backend"
                        )
                except Exception as e:
                    migration_results[domain] = False
            ...
            logger.info(
                f"Migration completed: {successful}/{len(domains)} certificates migrated successfully"
            )
