...
class AuthManager:
    ...
    def __init__(self, settings_manager):
        ...
        if not BCRYPT_AVAILABLE:
            logger.warning(
                "bcrypt not available, falling back to SHA-256 (less secure)"
            )
    ...
    def validate_session(self, session_id):
        ...
        return {"username": session_data["user"], "role": session_data["role"]}
        ...
        def decorated_function(*args, **kwargs):
            try:
                ...
                if not secrets.compare_digest(token, expected_token):
                    logger.warning(
                        f"Invalid API token attempt from {request.remote_addr}"
                    )
                    return {
