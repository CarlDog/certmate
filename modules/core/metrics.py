...
total_domains = Gauge("certmate_domains_total", "Total number of managed domains")

total_certificates = Gauge(
    "certmate_certificates_total", "Total number of certificates"
)
...
certificates_by_status = Gauge(
    "certmate_certificates_by_status", "Number of certificates by status", ["status"]
)
...
application_uptime = Gauge(
    "certmate_application_uptime_seconds", "Application uptime in seconds"
)
...
# Cache metrics
cache_hits_total = Counter("certmate_cache_hits_total", "Total number of cache hits")

cache_misses_total = Counter(
    "certmate_cache_misses_total", "Total number of cache misses"
)

cache_entries = Gauge("certmate_cache_entries", "Number of entries in cache")
...
class CertMateMetricsCollector:
    ...
    def __init__(self):
        ...
        if PROMETHEUS_AVAILABLE:
            ...
            try:
                ...
                logger.debug(
                    f"Info metric not supported in this prometheus_client version: {e}"
                )
                try:
                ...
                if cert_info.get("exists", False):
                    ...
                    if days_left is not None:
                        certificate_expiry_days.labels(
                            domain=domain, dns_provider=dns_provider
                        ).set(max(0, days_left))
                    ...
                    certificate_last_renewal.labels(
                        ...
                        time.time() - (renewal_threshold_days - days_left) * 24 * 3600
                        if days_left is not None
                        else 0
                    )
                    ...
                    certificate_next_renewal.labels(
                        ...
                        time.time() + (days_left - renewal_threshold_days) * 24 * 3600
                        if days_left is not None and days_left > renewal_threshold_days
                        else time.time()
                    )
    ...
    def record_certificate_request(self, domain: str, dns_provider: str, success: bool):
        ...
        certificate_requests_total.labels(
            domain=domain, dns_provider=dns_provider, status=status
        ).inc()
    ...
    def record_certificate_creation_time(self, dns_provider: str, duration: float):
        ...
        certificate_creation_duration.labels(dns_provider=dns_provider).observe(
            duration
        )
    ...
    def record_acme_error(self, error_type: str, domain: str, dns_provider: str):
        ...
        acme_errors_total.labels(
            error_type=error_type, domain=domain, dns_provider=dns_provider
        ).inc()

    def record_rate_limit_hit(self, limit_type: str, dns_provider: str):
        """Record a rate limit hit."""
        acme_rate_limit_hits.labels(
            limit_type=limit_type, dns_provider=dns_provider
        ).inc()
