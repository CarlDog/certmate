...
class CertificateManager:
    ...
    def get_certificate_info(self, domain):
        ...
        if self.storage_manager:
            try:
                ...
                logger.warning(
                    f"Failed to retrieve certificate from storage backend for {domain}: {e}"
                )
        ...
        if metadata_file.exists():
            try:
                ...
                with open(metadata_file, "r") as f:
                    ...
                    logger.debug(
                        f"Found DNS provider '{dns_provider}' in metadata for {domain}"
                    )
            except Exception as e:
        ...
        if not dns_provider:
            ...
            dns_provider = self.settings_manager.get_domain_dns_provider(
                domain, settings
            )
            logger.debug(
                f"Using DNS provider '{dns_provider}' from settings for {domain}"
            )
    ...
    def create_certificate(
        ...
        try:
            ...
            if self.ca_manager:
                try:
                    ca_account_config, used_ca_account_id = (
                        self.ca_manager.get_ca_config(ca_provider, ca_account_id)
                    )
                    logger.info(f"Using CA account: {used_ca_account_id}")
                except Exception as e:
                    logger.warning(
                        f"Could not get CA config, using default Let's Encrypt: {e}"
                    )
            ...
            if not dns_config:
                ...
                if not dns_config:
                    raise ValueError(
                        f"DNS provider '{dns_provider}' account '{account_id or 'default'}' not configured"
                    )

                logger.info(
                    f"Using DNS provider: {dns_provider} with account: {used_account_id}"
                )
            ...
            if san_domains:
                ...
                logger.info(
                    f"Creating SAN certificate with domains: {', '.join(all_domains)}"
                )
            ...
            if self.ca_manager and ca_account_config:
                ...
                certbot_cmd = [
                    ...
                    str(cert_output_dir),
                    "--work-dir",
                    str(cert_output_dir / "work"),
                    "--logs-dir",
                    str(cert_output_dir / "logs"),
                ]
            ...
            strategy.configure_certbot_arguments(
                certbot_cmd, credentials_file, domain_alias=domain_alias
            )
            ...
            certbot_cmd.extend(
                [f"--{strategy.plugin_name}-propagation-seconds", str(propagation_time)]
            )

            logger.info(f"Running certbot command for {domain} with {dns_provider}")
            logger.debug(
                f"Certbot command: {' '.join(str(item) for item in certbot_cmd)}"
            )
            ...
            if self.storage_manager:
                try:
                    storage_success = self.storage_manager.store_certificate(
                        domain, cert_files, metadata
                    )
                    if storage_success:
                        logger.info(
                            f"Certificate stored in {self.storage_manager.get_backend_name()} backend for {domain}"
                        )
                    else:
                        logger.warning(
                            f"Failed to store certificate in {self.storage_manager.get_backend_name()} backend for {domain}"
                        )
                except Exception as e:
                    logger.error(
                        f"Error storing certificate in storage backend for {domain}: {e}"
                    )
            ...
            logger.info(
                f"Certificate created successfully for {domain} in {duration:.2f} seconds"
            )
            ...
            if env_vars_set:
                ...
                try:
                    strategy = (
                        DNSStrategyFactory.get_strategy(dns_provider)
                        if "dns_provider" in locals() and dns_provider
                        else None
                    )
                    if strategy:
                        strategy.cleanup_environment(os.environ)
            ...
            duration = time.time() - start_time
            logger.error(
                f"Certificate creation failed for {domain}: {str(e)} (duration: {duration:.2f}s)"
            )
            raise
    ...
    def renew_certificate(self, domain):
        ...
        try:
                ...
                if metadata_file.exists():
                    try:
                        ...
                        with open(metadata_file, "w") as f:
                            json.dump(metadata, f, indent=2)
                        logger.info(
                            f"Updated renewal timestamp in metadata for {domain}"
                        )
                    except Exception as e:
            ...
            error_msg = str(e)
            logger.error(
                f"Exception during certificate renewal for {domain}: {error_msg}"
            )
    ...
    def check_renewals(self):
        ...
        for domain_entry in settings.get("domains", []):
            try:
                ...
                logger.error(
                    f"Error checking renewal for domain entry {domain_entry}: {e}"
                )
    ...
    def _get_dns_config(self, dns_provider, account_id):
        ...
        return self.dns_manager.get_dns_provider_account_config(
            dns_provider, account_id
        )
        ...
        for domain_dir in cert_dir.iterdir():
            ...
            try:
                ...
                with open(metadata_file, "w") as f:
                    json.dump(metadata, f, indent=2)
                logger.info(
                    f"Created metadata for {domain} with inferred DNS provider: {dns_provider}"
                )
                created_count += 1
            except Exception as e:
