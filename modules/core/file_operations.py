...
class FileOperations:
    ...
    def safe_file_read(self, file_path, is_json=False, default=None):
        ...
        try:
            ...
            if not any(
                ...
                logger.error(
                    f"Access denied: file outside allowed directories: {file_path}"
                )
                return default
        ...
        try:
            ...
            metadata = {
                ...
                "settings_domains": [
                    ...
                    for d in settings_data.get("domains", [])
                ],
            ...
            with zipfile.ZipFile(backup_path, "w", zipfile.ZIP_DEFLATED) as zipf:
                # Add settings data
                settings_backup = {"metadata": metadata, "settings": settings_data}
            ...
            logger.info(
                f"Unified backup created: {backup_filename} (contains {len(domains)} domains)"
            )
                ...
                for backup_file in unified_backup_dir.glob("backup_*.zip"):
                    try:
                        ...
                        backups["unified"].append(
                            {"filename": backup_file.name, "metadata": metadata}
                        )
                    except Exception as e:
                        logger.warning(
                            f"Could not process unified backup {backup_file}: {e}"
                        )
    ...
    def restore_unified_backup(self, backup_file_path):
        ...
        try:
            ...
            with zipfile.ZipFile(backup_path, "r") as zipf:
                ...
                if "settings.json" in zipf.namelist():
                    ...
                    if "settings" in settings_backup:
                        ...
                        if self.safe_file_write(
                            settings_file, settings_data, is_json=True
                        ):
                            logger.info("Settings restored from unified backup")
                        else:
                            logger.error(
                                "Failed to restore settings from unified backup"
                            )
                            return False
                ...
                for file_info in zipf.infolist():
                    if (
                        file_info.filename.startswith("certificates/")
                        and file_info.filename != "certificates/"
                    ):
                        logger.info(
                            f"Extracting certificate file: {file_info.filename}"
                        )
                        ...
                        try:
                            with zipf.open(file_info) as source, open(
                                target_path, "wb"
                            ) as target:
            ...
            logger.info(
                f"Unified backup restored successfully from: {backup_path.name}"
            )
            if restored_domains:
                logger.info(
                    f"Restored {len(restored_domains)} domains: {', '.join(restored_domains)}"
                )
            return True
