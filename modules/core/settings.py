...
class SettingsManager:
    ...
    def load_settings(self):
        ...
        try:
            ...
            if settings is None:
                logger.warning(
                    "Settings file exists but is empty or corrupted, recreating with defaults"
                )
                self._save_settings_compat(first_time_template)
                return first_time_template
            ...
            if "api_bearer_token" in settings:
                ...
                try:
                    ...
                    if hasattr(app, "validate_api_token"):
                        is_valid, token_or_error = app.validate_api_token(
                    ...
                    if isinstance(domain_entry, str):
                        ...
                        try:
                            ...
                            if hasattr(app, "validate_domain"):
                                is_valid, domain_or_error = app.validate_domain(
                                    domain_entry
                                )
                            else:
            ...
            for cert_path in cert_dir.iterdir():
                if cert_path.is_dir():
                    ...
                    if not metadata_file.exists():
                        ...
                        dns_provider = self._get_domain_provider_from_settings(
                            domain, settings
                        )
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Failed to create metadata for {domain}: {e}"
                            )
