...
_MULTI_PROVIDER_TEMPLATE_MAP = {
    ...
    "dnsmadeeasy": {
        "dns_dnsmadeeasy_api_key": "api_key",
        "dns_dnsmadeeasy_secret_key": "secret_key",
    },
    ...
    "porkbun": {
        "dns_porkbun_api_key": "api_key",
        "dns_porkbun_secret_key": "secret_key",
    },
...
def validate_api_token(token: str) -> Tuple[bool, str]:
    ...
    if not (_MIN_TOKEN_LENGTH <= len(token) <= _MAX_TOKEN_LENGTH):
        return (
            False,
            f"API token length must be between {_MIN_TOKEN_LENGTH} and {_MAX_TOKEN_LENGTH} characters.",
        )
    ...
    if unique_chars < _MIN_UNIQUE_CHARS:
        return (
            False,
            f"API token lacks character variety (must have at least {_MIN_UNIQUE_CHARS} unique characters).",
        )
    ...
    if char_types < 2:
        return (
            False,
            "API token must contain at least 2 character types (uppercase, lowercase, digits).",
        )
...
def generate_secure_token(length: int = 40) -> str:
    ...
    if not isinstance(length, int) or length < _MIN_TOKEN_LENGTH:
        raise ValueError(
            f"Token length must be an integer of at least {_MIN_TOKEN_LENGTH} characters for security."
        )
    ...
    raise RuntimeError(
        f"Failed to generate a valid token after {max_attempts} attempts"
    )
...
def create_digitalocean_config(api_token: str) -> Path:
    ...
    return _create_config_file(
        "digitalocean", f"dns_digitalocean_token = {api_token}\n"
    )
...
def create_namecheap_config(username: str, api_key: str) -> Path:
    ...
    content = (
        f"dns_namecheap_username = {username}\ndns_namecheap_api_key = {api_key}\n"
    )
...
def create_acme_dns_config(
    api_url: str, username: str, password: str, subdomain: str
) -> Path:
...
def validate_dns_provider_account(
    provider: str, account_id: str, account_config: Dict[str, Any]
) -> Tuple[bool, str]:
    """
    ...
    try:
        ...
        if not isinstance(account_config, dict):
            return (
                False,
                f"Account configuration must be a dictionary, but got {type(account_config).__name__}.",
            )

        required_fields = _DNS_PROVIDER_CREDENTIALS[provider]
        missing_fields = [
            f for f in required_fields if not str(account_config.get(f) or "").strip()
        ]
