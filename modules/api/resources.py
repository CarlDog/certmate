            ...
            try:
                ...
                if san_domains:
                    ...
                    for san in san_domains:
                        ...
                        if san and (
                            san.startswith("http://") or san.startswith("https://")
                        ):
                            return {
                ...
                if not domain_exists:
                    ...
                    logger.info(
                        f"Added domain {domain} to settings after certificate creation"
                    )
                ...
                return {"error": error_msg, "hint": hint}, 400
        ...
        @api.expect(
            api.model(
                "BackupCreateRequest",
                {
                    "type": fields.String(
                        required=True,
                        enum=["unified", "settings", "certificates", "both"],
                        description="Type of backup to create (unified recommended for data consistency)",
                    ),
                    "reason": fields.String(
                        description="Reason for backup creation", default="manual"
                    ),
                },
            )
        )
            ...
            try:
                ...
                if created_backups:
                    return {
                        ...
                        "recommendation": "Use unified backups for data consistency"
    ...
    class BackupRestore(Resource):
            ...
            api.model(
                "BackupRestoreRequest",
                {
                    "filename": fields.String(
                        required=True, description="Backup filename to restore from"
                    ),
                    "create_backup_before_restore": fields.Boolean(
        ...
        def post(self, backup_type):
            ...
            try:
                if backup_type != "unified":
                    return {
                        "error": "Only unified backup restoration is supported"
                    }, 400
                ...
                if success:
                    ...
                    if pre_restore_backup:
                        ...
                        response["note"] = (
                            "A backup of the previous state was created before restore"
                        )
        ...
        def post(self):
            ...
            try:
                ...
                success = settings_manager.save_settings(
                    settings, backup_reason="storage_backend_update"
                )
            ...
            try:
                ...
                try:
                    if ca_provider == "letsencrypt":
                        ...
                        directory_url = ca_manager._get_letsencrypt_directory_url(
                            environment
                        )
                        ...
                        if not (
                            acme_url.startswith("http://")
                            or acme_url.startswith("https://")
                        ):
                            return {
                                "success": False,
                        ...
                        if ca_cert and not (
                            ca_cert.strip().startswith("-----BEGIN CERTIFICATE-----")
                            and ca_cert.strip().endswith("-----END CERTIFICATE-----")
                        ):
                            return {
                                "success": False,
                        ...
                        try:
                            ...
                            if ca_cert:
                                ...
                                logger.info(
                                    "Custom CA certificate provided for Private CA"
                                )

                            response = requests.get(
                                acme_url, timeout=timeout, verify=verify_ssl
                            )
                            ...
                            if response.status_code == 200:
                                try:
                                    ...
                                    if (
                                        "newAccount" in directory_data
                                        or "keyChange" in directory_data
                                    ):
                                        return {
                                            "success": True,
                                            "message": "Private CA ACME endpoint is accessible and appears to be a valid ACME directory",
                                            "ca_provider": ca_provider,
                                            "acme_url": acme_url,
    ...
    class StorageBackendMigrate(Resource):
        ...
        def post(self):
            ...
            try:
                ...
                if (
                    source_backend_type not in backend_classes
                    or target_backend_type not in backend_classes
                ):
                    return {"error": "Invalid backend type"}, 400
                ...
                try:
                    ...
                    if source_backend_type == "local_filesystem":
                        source_backend = LocalFileSystemBackend(
                            Path(source_config.get("cert_dir", "certificates"))
                        )
                    else:
                        source_backend = backend_classes[source_backend_type](
                            source_config
                        )

                    if target_backend_type == "local_filesystem":
                        target_backend = LocalFileSystemBackend(
                            Path(target_config.get("cert_dir", "certificates"))
                        )
                    else:
                        target_backend = backend_classes[target_backend_type](
                            target_config
                        )
                    ...
                    successful = sum(
                        1 for success in migration_results.values() if success
                    )
