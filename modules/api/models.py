...
def create_api_models(api):
    ...
    digitalocean_model = api.model(
        "DigitalOceanConfig",
        {"api_token": MaskedString(description="DigitalOcean API token")},
    )

    linode_model = api.model(
        "LinodeConfig", {"api_key": MaskedString(description="Linode API key")}
    )

    gandi_model = api.model(
        "GandiConfig", {"api_token": MaskedString(description="Gandi API token")}
    )
    ...
    he_ddns_model = api.model(
        "HurricaneElectricConfig",
        {
    ...
    dynudns_model = api.model(
        "DynuConfig", {"token": MaskedString(description="Dynu API token")}
    )

    arvancloud_model = api.model(
        "ArvanCloudConfig", {"api_key": MaskedString(description="ArvanCloud API key")}
    )

    acme_dns_model = api.model(
        "ACMEDNSConfig",
        {
    ...
    multi_provider_model = api.model(
        "MultiProviderConfig",
        {
            "provider": fields.String(
                description="DNS provider name (e.g., hetzner, porkbun, vultr)"
            ),
            "config": fields.Raw(
                description="Provider-specific configuration (flexible key-value pairs)"
            ),
        },
    )

    dns_providers_model = api.model(
        "DNSProviders",
        {
            ...
            "multi": fields.Raw(
                description="Configuration for any DNS provider via certbot-dns-multi"
            ),
        },
    )
    ...
    settings_model = api.model(
        "Settings",
        {
            "cloudflare_token": MaskedString(
                description="Cloudflare API token (deprecated, use dns_providers)"
            ),
            "domains": fields.List(
                fields.Raw, description="List of domains (can be strings or objects)"
            ),
            "email": fields.String(description="Email for Let's Encrypt"),
            "auto_renew": fields.Boolean(description="Enable auto-renewal"),
            "api_bearer_token": MaskedString(
            ...
            "dns_providers": fields.Nested(
                dns_providers_model, description="DNS provider configurations"
            ),
        },
    )

    create_cert_model = api.model(
        "CreateCertificate",
        {
            "domain": fields.String(
                required=True,
                description="Primary domain name to create certificate for",
            ),
            "san_domains": fields.List(
                fields.String,
                description='Additional Subject Alternative Names (SANs) - e.g., ["www.example.com", "mail.example.com", "*.example.com"]',
            ),
            "dns_provider": fields.String(
            ...
            "account_id": fields.String(
                description="DNS provider account ID to use (optional, uses default account if not specified)"
            ),
            "ca_provider": fields.String(
                description="Certificate Authority provider to use (optional, uses default from settings)",
                enum=["letsencrypt", "digicert", "private_ca"],
            ),
            "domain_alias": fields.String(
                description="Optional domain alias for DNS validation (e.g., _acme-challenge.validation.example.org)"
            ),
        },
    )

    # Cache models
    cache_entry_model = api.model(
        "CacheEntry",
        {
            ...
            "status": fields.String(
                description="Deployment status", enum=["deployed", "not-deployed"]
            ),
        },
    )

    cache_stats_model = api.model(
        "CacheStats",
        {
            ...
            "entries": fields.List(
                fields.Nested(cache_entry_model), description="List of cached entries"
            ),
        },
    )

    cache_clear_response_model = api.model(
        "CacheClearResponse",
        {
            ...
            "cleared_entries": fields.Integer(
    ...
    # Backup models
    backup_metadata_model = api.model(
        "BackupMetadata",
        {
    ...
    backup_list_model = api.model(
        "BackupList",
        {
            "unified": fields.List(
                fields.Nested(backup_metadata_model), description="Unified backups"
            )
        },
    )

    # Storage Backend models
    azure_keyvault_storage_model = api.model(
    ...
    aws_secrets_manager_storage_model = api.model(
    ...
    hashicorp_vault_storage_model = api.model(
        "HashiCorpVaultStorage",
        {
            ...
            "mount_point": fields.String(
                description="Vault Mount Point", default="secret"
            ),
            "engine_version": fields.String(
                description="KV Engine Version", default="v2"
            ),
        },
    )

    infisical_storage_model = api.model(
        "InfisicalStorage",
        {
            "site_url": fields.String(
                description="Infisical Site URL", default="https://app.infisical.com"
            ),
            ...
            "environment": fields.String(
                description="Infisical Environment", default="prod"
            ),
        },
    )

    storage_config_model = api.model(
        "StorageConfig",
        {
            ...
            "cert_dir": fields.String(
                description="Certificate directory for local filesystem"
            ),
    ...
    storage_test_config_model = api.model(
        "StorageTestConfig",
        {
            "backend": fields.String(
                description="Storage backend type to test", required=True
            ),
            "config": fields.Raw(
                description="Backend-specific configuration", required=True
            ),
        },
    )

    storage_migration_config_model = api.model(
        "StorageMigrationConfig",
        {
            "source_backend": fields.String(
                description="Source storage backend type", required=True
            ),
            "target_backend": fields.String(
                description="Target storage backend type", required=True
            ),
            "source_config": fields.Raw(
                description="Source backend configuration", required=True
            ),
            "target_config": fields.Raw(
                description="Target backend configuration", required=True
            ),
        },
    )

    # CA Provider models
    ca_test_config_model = api.model(
        "CATestConfig",
        {
            "ca_provider": fields.String(
                description="CA provider type to test", required=True
            ),
            "config": fields.Raw(
                description="CA provider-specific configuration", required=True
            ),
        },
    )

    # Client Certificate models
    client_certificate_model = api.model(
        "ClientCertificate",
        {
    ...
    client_certificate_request_model = api.model(
    ...
    client_certificate_revoke_model = api.model(
        "ClientCertificateRevoke",
