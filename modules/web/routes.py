...
def register_web_routes(app, managers):
    ...
    def index():
        ...
        try:
            ...
            return render_template(
                "index.html", certificates=certificates, api_token=api_token
            )
        ...
        if request.method == "GET":
            ...
            try:
                ...
                if enable and not auth_manager.has_any_users():
                    return jsonify(
                        {
                            "error": "Create at least one admin user before enabling local auth"
                        }
                    ), 400
                ...
                if auth_manager.enable_local_auth(enable):
                    return jsonify(
                        {
                            "message": f'Local authentication {"enabled" if enable else "disabled"}'
                        }
                    )
                else:
    ...
    def web_settings():
        ...
        if request.method == "GET":
            ...
            try:
                ...
                if not setup_completed and all(
                    key in merged_settings and merged_settings[key]
    ...
    def web_dns_provider_accounts(provider):
        ...
        if request.method == "GET":
            ...
            try:
                ...
                success = dns_manager.create_dns_account(
                    provider, account_id, account_config
                )

                if success:
                    return jsonify(
                        {
                            "message": f"DNS account {account_id} created/updated successfully"
                        }
                    )
                else:
                    return jsonify(
                        {"error": "Failed to create/update DNS account"}
                    ), 500
    ...
    @app.route(
        "/api/dns/<string:provider>/accounts/<string:account_id>",
    ...
    def web_dns_provider_account(provider, account_id):
        ...
        if request.method == "GET":
            try:
                config, _ = dns_manager.get_dns_provider_account_config(
                    provider, account_id
                )
                if config:
                ...
                logger.error(
                    f"Error getting DNS account {account_id} for {provider}: {e}"
                )
            ...
            try:
                ...
                if success:
                    return jsonify(
                        {"message": f"DNS account {account_id} updated successfully"}
                    )
                else:
                ...
                logger.error(
                    f"Error updating DNS account {account_id} for {provider}: {e}"
                )
                return jsonify({"error": "Failed to update DNS account"}), 500
            ...
            try:
                ...
                if success:
                    return jsonify(
                        {"message": f"DNS account {account_id} deleted successfully"}
                    )
                else:
                ...
                logger.error(
                    f"Error deleting DNS account {account_id} for {provider}: {e}"
                )
                return jsonify({"error": "Failed to delete DNS account"}), 500
    ...
    def web_create_certificate():
        ...
        try:
            ...
            san_domains_raw = data.get(
                "san_domains", ""
            )  # Can be comma-separated string or list
            dns_provider = data.get(
                "dns_provider"
            )  # Optional, uses default from settings
            ...
            if san_domains_raw:
                if isinstance(san_domains_raw, list):
                    ...
                    san_domains = [
                        d.strip() for d in san_domains_raw.split(",") if d.strip()
                    ]
            ...
            if not dns_provider:
                dns_provider = settings_manager.get_domain_dns_provider(
                    domain, settings
                )
            ...
            dns_providers_config.get(dns_provider, {})
            ...
            if account_id:
                ...
                if not config:
                    available_accounts = list(
                        dns_manager.list_dns_provider_accounts(dns_provider).keys()
                    )
                    hint = (
                        f"Available accounts: {', '.join(available_accounts)}"
                        if available_accounts
                        else "Configure a DNS account in Settings first."
                    )
                    return jsonify(
                        {
            ...
            def create_cert_async():
                try:
                    ...
                    domains_info = f"{domain}" + (
                        f" (+ {len(san_domains)} SANs)" if san_domains else ""
                    )
                    logger.info(
                        f"Background certificate creation completed for {domains_info}"
                    )
                except Exception as e:
                    logger.error(
                        f"Background certificate creation failed for {domain}: {e}"
                    )
        ...
        try:
            ...
            def renew_cert_async():
                try:
                    ...
                    logger.info(
                        f"Background certificate renewal completed for {domain}"
                    )
                except Exception as e:
                    logger.error(
                        f"Background certificate renewal failed for {domain}: {e}"
                    )
            ...
            return jsonify(
                {
                    "success": True,
                    "message": f"Certificate renewal started for {domain}",
                }
            )
    ...
    def web_download_backup(backup_type, filename):
        ...
        try:
            if backup_type != "unified":
                return jsonify(
                    {"error": "Only unified backup download is supported"}
                ), 400
            ...
            # Security check
            if not str(backup_path.resolve()).startswith(
                str(Path(file_ops.backup_dir).resolve())
            ):
